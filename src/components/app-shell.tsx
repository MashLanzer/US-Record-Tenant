...
const a11yCopy = {
  en: { skip: "Skip to content", primary: "Primary", main: "Main content" },
  es: { skip: "Saltar al contenido", primary: "Principal", main: "Contenido principal" },
};

export function AppShell({ children }: { children: ReactNode }) {
  const pathname = usePathname();
  const t = useT(common);
  const a = useT(a11yCopy);
  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-[1180px]">
      ...
      <aside className="sticky top-0 hidden h-dvh w-[248px] shrink-0 flex-col border-r border-line px-4 py-6 lg:flex">
        ...
        <nav className="flex flex-col gap-1" aria-label={a.primary}>
      ...
      {/* Main content */}
      <main id="main" className="min-w-0 flex-1" aria-label={a.main}>
        <OfflineBanner />
        <div className="mx-auto w-full max-w-[640px] pb-28 lg:pb-12">{children}</div>
      </main>

      {/* Mobile bottom tab bar */}
      <nav className="safe-bottom fixed inset-x-0 bottom-0 z-40 flex items-center justify-around border-t border-line bg-surface/90 px-2 pt-2 backdrop-blur-xl lg:hidden" aria-label={a.primary}>
        {NAV.map(({ key, href, icon: Icon }) => {
          const active = isActive(pathname, href);
          return (
            <Link
